estimate_gps<-function(trtobs,confounders,trtcf,trtcf_rev){
  ...
  for (j in 1:ncol(trtobs)){
    if (j==1){
      ...
      gps_obs1=gps_obs1*dnorm(trtobs[(n+1):N,j],mean=mod_mean,sd=mod_sd)
      gps_cf1=gps_cf1*dnorm(trtcf[(n+1):N,j],mean=mod_mean,sd=mod_sd)
      gps_rcf1=gps_rcf1*dnorm(trtcf_rev[(n+1):N,j],mean=mod_mean,sd=mod_sd)
      ...
      gps_obs2=gps_obs2*dnorm(trtobs[1:n,j],mean=mod_mean,sd=mod_sd)
      gps_cf2=gps_cf2*dnorm(trtcf[1:n,j],mean=mod_mean,sd=mod_sd)
      gps_rcf2=gps_rcf2*dnorm(trtcf_rev[1:n,j],mean=mod_mean,sd=mod_sd)
    }
  }
