### NON-GAUSSION LINEAR IN ASSIGNMENT MODEL, NON-LINEAR IN OUTCOME MODEL
library(mvtnorm)
library(dplyr)
...
random_term=matrix(rpois(3*N,10),ncol=3)+matrix(runif(3*N,0,1),ncol=3)
beta=matrix(rep(c(rep(1,3),rep(0.2,7)),3),nrow=10,byrow=F)
Eobs=X%*%beta+random_term

unit=1.5*apply(Eobs,2,sd)
...
#print(length(subp))
save(N,Eobs,Ecf,Ercf,unit,X,Yobs,real_dif,man_dif,subp,file="working_data/dataset2.Rdata")
