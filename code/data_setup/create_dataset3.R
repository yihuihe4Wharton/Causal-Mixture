### GAUSSION NON-LINEAR IN ASSIGNMENT MODEL, NON-LINEAR IN OUTCOME MODEL
library(mvtnorm)
library(dplyr)
library(GA)
library(truncnorm)
...
Eobs=X%*%beta+(X**2)%*%beta+random_term

unit=apply(Eobs,2,sd)/3
...
#print(length(subp))
save(N,Eobs,Ecf,Ercf,unit,X,Yobs,real_dif,man_dif,subp,file="working_data/dataset3.Rdata")